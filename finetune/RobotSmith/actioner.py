...
from utils.peract_utils_robotsmith import (
    CAMERAS,
    SCENE_BOUNDS,
    IMAGE_SIZE,
)
...
class MyActioner(object):
    ...
    def predict(self, taskvar, episode_id, step_id, instruction, obs_state_dict):
        ...
        for idx, cam in enumerate(self.agent.cameras):
            obs_state_dict[f"{cam}_rgb"] = np.transpose(
                obs_state_dict["rgb"][idx], [2, 0, 1]
            )[None]
            obs_state_dict[f"{cam}_point_cloud"] = np.transpose(
                obs_state_dict["pcd"][idx], [2, 0, 1]
            )[None]

        del obs_state_dict["rgb"]
        del obs_state_dict["pcd"]

        for k, v in obs_state_dict.items():
            if isinstance(v, np.ndarray):
                v_copy = v.copy()
                obs_state_dict[k] = (
                    torch.from_numpy(v_copy).float().to(self.agent._device)
                )
            elif isinstance(v, list):
                obs_state_dict[k] = torch.tensor(v, dtype=torch.float32).to(
                    self.agent._device
                )
        ...
        return self.agent.act(
            step=step_id, observation=obs_state_dict, return_robotsmith_action=True
        )
