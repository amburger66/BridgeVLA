...
def train(
    ...
    def move_tensors_to_device(d, device):
        if isinstance(d, dict):
            return {
                k: (
                    move_tensors_to_device(v, device)
    ...
    iteration = 0
    epoch_losses = {}
    for raw_batch in tqdm.tqdm(
        data_loader, disable=(rank != 0), position=0, leave=True
    ):
        iteration += 1
...
def experiment(cmd_args):
    ...
    total_params = sum(
        p.numel() for p in agent._network.parameters() if p.requires_grad
    )
    ...
    if dist.get_rank() == 0:
        ...
        if cmd_args.debug:
            ...
            wandb.init(
                entity="", project="RobotSmith-BridgeVLA", name=os.path.dirname(log_dir)
            )
    ...
    while True:
        ...
        out = train(
            agent, train_dataloader, rank=dist.get_rank(), cameras=cmd_args.cameras
        )
        if rank == 0:
