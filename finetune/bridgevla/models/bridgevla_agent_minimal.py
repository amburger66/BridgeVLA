...
def save_point_cloud_with_color(filename, points, colors, keypoint=None):
    ...
    with open(filename, "w") as f:
        ...
        for pt, clr in zip(points, colors):
            f.write(
                f"{pt[0]} {pt[1]} {pt[2]} {int(clr[0])} {int(clr[1])} {int(clr[2])}\n"
            )
...
def visualize_images(
    ...
    for i in range(3):
        ...
        Image.fromarray(gray_img, mode="L").save(
            os.path.join(save_dir, f"gray_{i}.png")
        )
...
class RVTAgent:
    def __init__(
        ...
        scene_bounds: list = robotsmith_utils.SCENE_BOUNDS,
        cameras: list = robotsmith_utils.CAMERAS,
        rot_ver: int = 0,
        rot_x_y_aug: int = 2,
        log_dir="",
    ):
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        params_to_optimize = filter(
            lambda p: p.requires_grad, self._network.parameters()
        )
    ...
    def update_gembench(
        ...
        action_ignore_collisions = (
            replay_sample["ignore_collisions"].unsqueeze(1).int()
        )  # (b, 1) of int
    ...
    def act(
        ...
        if visualize:
            ...
            visualize_images(
                mvt1_img, q_trans_1, save_dir=os.path.join(save_dir, "mvt1")
            )
            visualize_images(
                mvt2_img, q_trans_2, save_dir=os.path.join(save_dir, "mvt2")
            )
